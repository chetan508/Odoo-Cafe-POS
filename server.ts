...
app.get(["/api/employees", "/api/users"], requireAuth, requireRole(["admin"]), async (req, res) => {
  try {
...
app.post(["/api/employees", "/api/users"], requireAuth, requireRole(["admin"]), async (req, res) => {
  const { name, email, password, role } = req.body;
...
app.put(["/api/employees/:id", "/api/users/:id"], requireAuth, requireRole(["admin"]), async (req, res) => {
  try {
...
app.delete(["/api/employees/:id", "/api/users/:id"], requireAuth, requireRole(["admin"]), async (req, res) => {
  const user = (req as any).user;
...
app.post("/api/ai/forecast", requireAuth, async (req, res) => {
  try {
    ...
    try {
      const historyCount = await prisma.order.count({ where: { status: "paid" } });
