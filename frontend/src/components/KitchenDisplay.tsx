...
export default function KitchenDisplay({
  ...
  const getOrderStatusGroup = (o: Order): "to_cook" | "preparing" | "completed" => {
    ...
    const completedCount = kItems.filter((i) => i && i.completed).length;
      ...
      <div className="flex h-full w-full flex-col bg-[#FAF7F2] text-[#2B2B2B] font-sans">
        <header className="flex items-center justify-between border-b border-[#E6DDD2] bg-[#FFFFFF] px-6 py-4">
          <div className="flex items-center gap-3">
            <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-[#6F4E37] to-[#3E2723] shadow-md shadow-[#6F4E37]/15">
              <Coffee className="h-5 w-5 text-[#FAF7F2]" />
            </div>
            <div>
              <h1 className="font-display text-lg font-black text-[#3E2723]">KDS Screen</h1>
        ...
        <div className="flex-1 flex flex-col items-center justify-center bg-[#FAF7F2] text-[#6F4E37]">
          <div className="text-center font-display space-y-4">
            <div className="h-12 w-12 bg-white border border-[#E6DDD2] rounded-full animate-spin mx-auto flex items-center justify-center shadow-lg shadow-[#6F4E37]/10">
              <Coffee className="h-5 w-5 text-[#C8A96B]" />
            </div>
          ...
          {kItems.map((itm, index) => (
            <li key={index} className="flex items-start justify-between gap-2">
              <div className="flex items-start gap-2 max-w-[80%]">
                <span className="font-mono font-black text-[#C8A96B] text-sm">{itm.quantity}x</span>
                <div>
                  <span
                    className={`text-sm font-bold tracking-wide block ${
                      itm.completed ? "line-through text-gray-400" : "text-[#2B2B2B]"
                    }`}
                  >
                    {itm.productName}
                  </span>
                  {itm.notes && (
                    <span className="text-[10px] text-[#6F4E37] font-mono italic block mt-0.5">
                      ↳ {itm.notes}
                    </span>
                  )}
                </div>
