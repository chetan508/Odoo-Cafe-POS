...
export default function ReceiptModal({ order, customer, table, onClose }: ReceiptModalProps) {
  ...
  const upiVpa = "Cafe ODOO@ybl";
  const upiAmount = order.total.toFixed(2);
  const upiestr = `upi://pay?pa=${upiVpa}&pn=Cafe%20ODOO&am=${upiAmount}&cu=INR&tn=Order%20${order.orderNumber}`;
  ...
  const handleWhatsappShare = () => {
    ...
    const text = `Thanks for dining at Cafe ODOO! Your receipts for Order ${order.orderNumber} is ready. Total: ₹${order.total.toFixed(2)}. Have an awesome day!`;
